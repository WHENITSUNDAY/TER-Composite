...
void Element::computeKe(const Node& n1, const Node& n2, const Node& n3) {
    ...
    // Matrice de rigidité élémentaire : Ke = area * B^T * C * B
...
Node& Mesh::getNode(int id) {
    ...
    return _nodes[0]; // Fallback (non idéal)
}
...
Element& Mesh::getElement(int id) {
    ...
    return _elements[0]; // Fallback (non idéal)
}
...
void Mesh::initializeElements() {
    cout << "\nInitialisation des éléments..." << endl;
    ...
    cout << "Éléments valides: " << countValid << "/" << _elements.size() << endl;
    cout << "Aire totale: " << totalArea << " m²" << endl;
}